...
bool interactMark[2][maxRobots];
unsigned int robot_battery[2][maxRobots];
...
{
  double isInteract = false;
  glPushMatrix();
  glLoadIdentity();
  glTranslated(loc.x,loc.y,0);
  switch ( team ){
    case teamBlue:{
      if(interactMark[0][robotID]) isInteract = true;
      glColor3d(0.2549, 0.4941, 1.0);
      break;
    }
    case teamYellow:{
      if(interactMark[1][robotID]) isInteract = true;
      glColor3d(1.0, 0.9529, 0.2431);
      break;
    }
    default:{
  ...
  // barra de interação com a bola

  if(isInteract){
    glColor3d(1.0,0.0,0.0);
    // top corner
    drawQuad(-96, 90, 81, 96, RobotZ + 0.01);
    // left corner
    drawQuad(-96, 90, -90, -96, RobotZ + 0.01);
    // right corner
    drawQuad(80, 96, 86, -96, RobotZ + 0.01);
    // bottom corner
    drawQuad(-96, -90, 81, -96, RobotZ + 0.01);
  }

  // setando as cores de acordo com o time
  switch ( team ){
    case teamBlue:{
      glColor3d(0.0706, 0.2314, 0.6275);
      break;
  ...
  switch(team){
    case teamBlue:
      drawQuad(-96,124,(1.92 * robot_battery[0][robotID]) - 96.0,166,RobotZ+0.01);
      break;
    case teamYellow:
      drawQuad(-96,124,(1.92 * robot_battery[1][robotID]) - 96.0,166,RobotZ+0.01);
      break;
    default:
      break;
  }
...
void GLSoccerView::drawBall(vector2d loc)
{
  ...
  drawArc(loc,0,ball_radius,-M_PI,M_PI,BallZ);
  glColor3d(0.8706,0.3490,0.0);
  drawArc(loc,ball_radius-1,ball_radius+5,-M_PI,M_PI,BallZ);
...
void GLSoccerView::updateNewDetection(pacote& socketPacket)
{
  int numBlueRobots = socketPacket.qt_blue;
  int numYellowRobots = socketPacket.qt_yellow;
  int numBalls = 1;
  Robot robot;
  vector2d ball;
  ...
  for(int i=0; i<numBlueRobots; i++){
	  robot.loc.set(socketPacket.robots_blue[i].x, socketPacket.robots_blue[i].y);
    robot.id = socketPacket.robots_blue[i].id;
    robot.angle = DEG(socketPacket.robots_blue[i].angle);
    robot.team = teamBlue;
    robot.cameraID = cam;
  ...
  for(int i=0; i<numYellowRobots; i++){
	  robot.loc.set(socketPacket.robots_yellow[i].x, socketPacket.robots_yellow[i].y);
    robot.id = socketPacket.robots_yellow[i].id;
    robot.angle = DEG(socketPacket.robots_yellow[i].angle);
  ...
  for(int i=0; i<numBalls; i++){
    ball.set(socketPacket.ball.first, socketPacket.ball.second);
...
void GLSoccerView::updateFieldGeometry(const SSL_GeometryFieldSize& fieldSize) {
  ...
  //fieldDim.lines.push_back(new FieldLine("nome", 15, 0, 115, 0, 100));
